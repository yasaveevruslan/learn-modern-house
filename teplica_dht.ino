...
void setup() {
  ...
  Serial.println("Используется библиотека Servo.h");
}
...
void controlCooling(float temp) {
  bool needCooling = (temp > TEMP_TARGET + TEMP_HYSTERESIS);
...
void openServos() {
  ...
  servo2.write(0);
  servo3.write(0);
  Serial.println("СЕРВО ОТКРЫТЫ (180°)");
}
...
void closeServos() {
  ...
  servo2.write(90);
  servo3.write(90);
  Serial.println("🔒 СЕРВО ЗАКРЫТЫ (0°)");
}
...
void printDebug(int s1, int s2, int s3, int avg, float temp, int light) {
  ...
  Serial.print("Сервоприводы: ");
  Serial.println(servosOpen ? "ОТКРЫТЫ (180°)" : "ЗАКРЫТЫ (0°)");
